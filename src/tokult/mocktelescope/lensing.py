...
class GravLens(GridConverter):
    ...
    This class provides simple converter of the pixel grids. If you want to use misc
    functions to treat lensing effects, use a class XXX instead.

    Attributes:
        lensmap [np.ndarray]: Array including pixel deflection maps with shape of
            (ny, nx, 2), which is the same as the original (input) grids. lensmap[:,:,0]
            includes x-direction and lensmap[:,:,1] includes y-direction.
    '''
    ...
    def gridconvert(self, grids: np.ndarray) -> np.ndarray:
        ...
        if grids.shape != self.pixel_deflect.shape:
            msg = (
                f'The input grid shape is {grids.shape}, '
                f'but the shape of the input lens model is {self.pixel_deflect.shape}.'
                'These two data must have the same shape.'
            )
            logger.error(msg)
